...
Item {
  ...
  Item {
    anchors.fill: parent
    visible: !root.problem || root.problem.operation !== "divide"
    Repeater {
      model: root.columns
      delegate: Text {
  ...
  DivisionBoard {
    anchors.fill: parent
    visible: root.problem !== null && root.problem.operation === "divide"
    problem: root.problem; board: root.board; step: root.step
  }
}
