...
FocusScope {
  ...
  readonly property bool valid: addition.valid && subtraction.valid
  readonly property bool canSave: available && loaded && dirty && valid && !busy && retrySeconds === 0 && password.text.length > 0
  signal saveRequested(var limits, string password)
  signal closeRequested()
  signal retryRequested()
  function clear() {
    password.text = ""; feedback = ""; failed = false; loaded = false; dirty = false
  }
  ...
  function submit() {
    ...
    saveRequested({add: addition.limit, subtract: subtraction.limit}, secret)
  }
  function finish(result) {
    if (result.ok) {
      load(result); feedback = "Limits saved. Today's completed totals are kept."; failed = false
    } else {
      failed = true
  ...
  Text { x: 38; y: 64; text: root.status && root.status.user ? "Daily math practice for " + root.status.user : "Daily math practice"; color: "#927888"; font.pixelSize: 16 }
  ...
  Text {
    x: 38; y: 111; width: 980; color: "#806C7C"; font.pixelSize: 15
    text: "Limits cover every difficulty and Mixed practice. Multiplication and division stay available."
  }
  PracticeLimitRow {
    id: addition; objectName: "additionLimits"; x: 36; y: 148; width: 1012
  ...
  PracticeLimitRow {
    id: subtraction; objectName: "subtractionLimits"; x: 36; y: 290; width: 1012
  ...
  Rectangle {
    ...
    HotelButton {
      ...
      text: root.busy ? "Checking password…" : root.retrySeconds > 0 ? "Try again in " + root.retrySeconds + "s" : "Save limits"
    ...
    Text {
      ...
      text: root.busy ? "Checking the parent password and saving your changes…"
        : root.feedback || (!root.valid ? "Enter a daily limit from 1 to 10,000." : "Only completed problems count. New allowances start each day; today's progress is kept.")
