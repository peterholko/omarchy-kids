...
These adapters publish six independent root plugins from Omarchy Kids without changing the built-in Kids packages. They target the stock Omarchy Quattro plugin API and use IDs under `io.github.peterholko.*`.
...
Screen Time and School Mode include byte-identical copies of the shared controls service. It installs separately through the explicit `sudo ./setup --user USER` step documented in their generated READMEs. The service has separate password, configuration, runtime and state paths; it does not alter OS accounts or adopt existing Omarchy Kids policies. School Mode's desktop effects require a second, unprivileged consent command. Keep the shared service `VERSION` synchronized across both releases; changing an installed service requires an explicit upgrade.
