...
Without Screen Time, practice remains available. Installing or removing School Mode, DNS or browsing logging does not change the package dependencies. To allow practice during school, open the School Mode pill → **School settings** gear, enter the parent password, and turn on **Number Grove** under **School apps**. It starts off unless already allowed and requires the Number Grove package to be installed. Its choice is independent of Paw Post and preserves other allowed apps and school hours. The command-line equivalent is `omarchy kids school apps add omarchy-number-grove --user CHILD_USERNAME`; use `remove` to revoke access. Without school access, the game window is hidden and paused during School Mode. With an exhausted time budget, the existing lock/Math Time flow remains in charge. Grove uses a normal application window and does not replace the lock screen or force an earning session.
...
This drives keyboard/mouse input through a real Qt Quick window, completes correct and incorrect rounds, checks movement timing and pause/focus behavior, resizes the view, and injects delayed, stale and capped reward replies. It also activates buttons with Tab/Space and focused mouse clicks, then checks that Space, Return and keypad Enter collect seeds after Play, Next seed, Resume and New round. Repeated collection keys while a reward request is pending must leave the request unchanged. It saves screenshots for inspection. It does not verify Quickshell IPC, Hyprland application filtering, the lock screen, package installation or ISO boot. Those integration checks still need an Omarchy laptop; no hosted build or ISO test is required to run the local checks.
...
The September 7 keyboard regression was reproduced with the Play button focused: Space restarted the round because focusing the surrounding scope restored that button. Gameplay now focuses a dedicated input item, following [Qt's focus-scope behavior](https://doc.qt.io/qt-6/qtquick-input-focus.html#acquiring-focus-and-focus-scopes). The focused engine suite and expanded Qt interaction harness passed, and the resulting seed collection and compact layout captures were reviewed.
