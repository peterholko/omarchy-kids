...
import socket
import threading
from omarchy_parent import API_VERSION, VERSION
    ...
    def __init__(self, layout, tick_seconds=5, log=print, modules=None):
        ...
        for name, package in allowed.items():
            if modules is not None and name not in modules:
                continue
            if importlib.util.find_spec("omarchy_parent." + package) is None:
                continue
            module = __import__("omarchy_parent." + package + ".service", fromlist=["Service"])
