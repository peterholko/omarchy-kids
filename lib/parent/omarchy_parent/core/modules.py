"""Package-backed first-party modules behind `omarchy parent plugin`."""
import argparse
import json
import os
import pwd
...
from omarchy_parent import VERSION
from . import session
...
class Manager:
    ...
    def install(self, names):
        ...
        subprocess.run(['systemctl', 'try-restart', 'omarchy-parent-timed.service'], check=True)
        ...
        for name, item in self.catalog.items():
            ...
            if installed:
                if name == 'core':
                    ...
                    unit = 'omarchy-parent-browsing.timer' if name == 'browsing' else 'omarchy-parent-timed.service'
                    if enabled:
                    ...
                    if enabled:
                        healthy = subprocess.run(['systemctl', 'is-active', '--quiet', 'omarchy-parent-dns.service']).returncode == 0
...
def main():
    ...
    if args.action != 'list' and os.geteuid() != 0:
        raise SystemExit('This operation needs the parent password; use omarchy parent plugin.')
