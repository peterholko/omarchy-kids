...
RUN_ROOT="${OMARCHY_PARENT_RUN:-$SYSROOT/run/user}"
...
install_browser_policies() {
  ...
  for file in "${FIREFOX_POLICY_FILES[@]}"; do
    [[ -f $file ]] || continue
    "$OMARCHY_PATH/bin/omarchy-parent-files" browser "$file" browsing "$fragment"
  done
}
...
remove_browser_policies() {
  ...
  for file in "${FIREFOX_POLICY_FILES[@]}"; do
    [[ -f $file ]] || continue
    "$OMARCHY_PATH/bin/omarchy-parent-files" browser "$file" browsing '{}'
  done
}
