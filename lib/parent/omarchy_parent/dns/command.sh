...
SYSROOT="${OMARCHY_PARENT_SYSROOT:-}"
...
RUN_DIR="$SYSROOT/run/omarchy-parent/dns"
...
dnsmasq_conf() {
  ...
  cat <<CONF
# Written by \`omarchy-parent dns\` from /etc/omarchy/parent.conf (dns=$mode,
...
CONF
  if [[ $upstream == "family" ]]; then
    ...
    echo "resolv-file=/run/omarchy-parent/dns/resolv.conf"
  fi
...
install_browser_policies() {
  ...
  for file in "${FIREFOX_POLICY_FILES[@]}"; do
    [[ -f $file ]] || continue
    "$OMARCHY_PATH/bin/omarchy-parent-files" browser "$file" dns "$fragment"
  done
}
...
remove_browser_policies() {
  ...
  for file in "${FIREFOX_POLICY_FILES[@]}"; do
    [[ -f $file ]] || continue
    "$OMARCHY_PATH/bin/omarchy-parent-files" browser "$file" dns '{}'
  done
}
