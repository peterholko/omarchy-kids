#!/bin/bash
#
# School mode and free time (shell/plugins/screen-time/school), after elgevan's
...
plugin="$ROOT/shell/plugins/screen-time/school"
...
python3 - "$plugin/../manifest.json" <<'PY' || fail "the manifest declares a menu, a bar widget, and a service under the combined plugin"
import json, sys
m = json.load(open(sys.argv[1]))
assert m["id"] == "omarchy.screen-time" and m["keepLoaded"] is True
assert set(m["kinds"]) == {"menu", "bar-widget", "service"}, m["kinds"]
assert m["entryPoints"] == {"menu": "Entry.qml", "barWidget": "BarWidget.qml", "service": "Service.qml"}
PY
...
const pluginId = "omarchy.school-mode", pillId = pluginId + ".mode", pillPath = "/usr/share/omarchy/shell/plugins/screen-time/school/ModePill.qml"
...
[[ -f $plugin/SchoolSettingsPage.qml ]] || fail "the school settings page is installed in the combined plugin"
[[ ! -f $plugin/manifest.json ]] || fail "school no longer registers a separate plugin"
pass "school mode is an internal part of the combined controls"

run_node_test <<'JS'
const fs = require('node:fs')
const vm = require('node:vm')
const source = fs.readFileSync(path.join(root, 'shell/plugins/screen-time/school/SchoolSettingsPage.qml'), 'utf8')
const Allowlist = require(path.join(root, 'shell/plugins/screen-time/school/Allowlist.js'))
const Schedule = require(path.join(root, 'shell/plugins/screen-time/school/SchoolSchedule.js'))
...
grep -q 'hl.bind("SUPER + SPACE", hl.dsp.exec_cmd(\[\[omarchy-shell shell toggle omarchy.screen-time' "$HYPR_LUA" || fail "the menu key opens the filtered menu"
...
grep -q 'ShellIntegration.activateCombined(config, root.pluginId, root.pluginDir + "/BarWidget.qml", root.schoolMode)' "$plugin/Service.qml" || fail "the service takes the menu slot and places the pill"
grep -q 'serviceFor("omarchy.screen-time")' "$plugin/Menu.qml" && grep -q '"/school-menu.jsonc"' "$plugin/Menu.qml" || fail "the menu wrapper filters by the service in school mode"
...
grep -q 'omarchy.screen-time' "$ROOT/bin/omarchy-kids-school" || fail "school setup installs the combined widget"
