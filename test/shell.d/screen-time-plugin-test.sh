...
assert sorted(m["kinds"]) == ["bar-widget", "menu", "service"], m["kinds"]
assert m["entryPoints"] == {"service": "Service.qml", "barWidget": "BarWidget.qml", "menu": "Entry.qml"}, m["entryPoints"]
...
for f in OverviewPage.qml TimeSettingsPage.qml; do
...
grep -q 'ParentPasswordField {' "$plugin/OverviewPage.qml" || fail "the parent drawer asks for the parent password"
grep -q 'root.service.shell.summon("omarchy.screen-time",' "$plugin/OverviewPage.qml" || fail "the panel opens Math time full screen"
! grep -q 'answerField' "$plugin/OverviewPage.qml" || fail "the panel no longer takes answers inline"
grep -q 'if (phase === "school") return iconBook' "$plugin/OverviewPage.qml" || fail "school time gets its own glyph"
grep -q '"earn": { "level": "grade" + (index + 1) }' "$plugin/TimeSettingsPage.qml" || fail "the settings pick a grade"
grep -Fq 'import "MathModel.js" as MathModel' "$plugin/TimeSettingsPage.qml" \
  && grep -Fq 'MathModel.gradeBlurb(MathModel.levelNumber(root.level))' "$plugin/TimeSettingsPage.qml" \
  && grep -Fq 'text: root.gradeBlurb' "$plugin/TimeSettingsPage.qml" \
  || fail "the parent settings and Math time share the same arithmetic-fact descriptions"
grep -q '"questions_per_set": value' "$plugin/TimeSettingsPage.qml" && grep -q '"set_minutes": value' "$plugin/TimeSettingsPage.qml" || fail "the settings set the questions and the minutes of a set"
grep -q 'togglePeriodDay' "$plugin/TimeSettingsPage.qml" && ! grep -q '"School time" : "Locks"' "$plugin/TimeSettingsPage.qml" || fail "time periods have days and cannot write school mode"
[[ ! -e $plugin/Countdown.qml ]] || fail "the bottom-centre countdown card is retired"
grep -q '| School & Screen Time | `omarchy.screen-time`' "$ROOT/shell/plugins/README.md" || fail "the plugin is listed"
...
grep -q 'shell.callIfLoaded("omarchy.screen-time", "mathOpen", "")' "$lock" && grep -q 'math: summoned, no time left and nothing on screen' "$lock" || fail "the lock screen re-opens Math time while there is no time"
...
grep -q 'session.shell_math_open(self.uid) is True' "$ROOT/lib/parent/omarchy_kids/screen_time/service.py" || fail "an open Math time session holds off the zero-budget relock"
